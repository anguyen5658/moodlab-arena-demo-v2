...
import {
  C, LOYALTY_TIERS, LOYALTY_BADGES, DAILY_CHALLENGES, DAILY_REWARDS, SHOP_ITEMS,
  GLASS_CARD, MOCK_FRIENDS, MOCK_COMMUNITIES, MOCK_COLLECTIONS
} from '../../constants'
...
export const MeTab: React.FC = () => {
  ...
  const _audio = useAudioContext()
  const ble = useBLEContext()

  const tier = player.getCurrentTier()
  const nextTier = LOYALTY_TIERS[tier.idx + 1] || null
  const winRate = player.playerProfile.gamesPlayed > 0
    ? Math.round((player.playerProfile.gamesWon / player.playerProfile.gamesPlayed) * 100)
    : 0
  const xpProgress = nextTier ? Math.min(((player.xp - tier.xpReq) / (nextTier.xpReq - tier.xpReq)) * 100, 100) : 100
  const totalMultiplier = player.getCoinMultiplier(ble.bleConnected)
  const earnedBadgeCount = player.earnedBadges.length
  const memberDays = Math.floor((Date.now() - (player.playerProfile.joinDate || Date.now())) / 86400000)
  const userLevel = Math.floor(player.xp / 500) + 1
  const avatar = player.ownedItems.includes("avatar_cat") ? "🐱" : player.ownedItems.includes("avatar_alien") ? "👽" : "🌟"

  const unreadMessages = 3
  const unreadNotifs = 5

  const ltPill = (id: string, label: string, color: string): React.CSSProperties => ({
    flex: 1, padding: "10px 0", borderRadius: 12, fontSize: 12, fontWeight: 700, cursor: "pointer", textAlign: "center",
    background: game.loyaltyTab === id ? `linear-gradient(135deg,${color || C.pink},${color || C.pink}cc)` : `rgba(255,255,255,0.04)`,
    color: game.loyaltyTab === id ? "#fff" : C.text3,
    border: game.loyaltyTab === id ? `1px solid ${color || C.pink}50` : `1px solid ${C.border}`,
    boxShadow: game.loyaltyTab === id ? `0 0 25px ${color || C.pink}40, 0 4px 16px rgba(0,0,0,0.35), inset 0 1px 0 rgba(255,255,255,0.15)` : "0 2px 8px rgba(0,0,0,0.2)",
    transition: "all 0.35s cubic-bezier(0.4,0,0.2,1)",
    transform: game.loyaltyTab === id ? "scale(1.03)" : "scale(1)",
    backdropFilter: game.loyaltyTab === id ? "none" : "blur(16px)",
    WebkitBackdropFilter: game.loyaltyTab === id ? "none" : "blur(16px)",
    position: "relative",
    letterSpacing: game.loyaltyTab === id ? 0.5 : 0,
  })
    ...
    <div style={{ padding: "0 14px", position: "relative" }}>
      ...
      <div style={{ padding: 20, borderRadius: 22, marginBottom: 16, position: "relative", zIndex: 1, overflow: "hidden",
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ marginTop: 6 }}>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 8, color: C.text3, marginBottom: 2 }}>
                <span style={{ fontWeight: 600 }}>{"XP " + player.xp.toLocaleString()}</span>
        ...
        <div style={{ display: "flex", gap: 10, marginTop: 14, alignItems: "center" }}>
          <div onClick={() => player.notify("Messages coming soon", C.cyan)} style={{ cursor: "pointer", position: "relative",
            padding: "6px 10px", borderRadius: 10, background: `rgba(255,255,255,0.05)`, border: `1px solid ${C.border}`,
            display: "flex", alignItems: "center", gap: 5, fontSize: 13, touchAction: "none" }}>
            {"💬"}
          ...
          <div onClick={() => player.notify("Notifications coming soon", C.orange)} style={{ cursor: "pointer", position: "relative",
            padding: "6px 10px", borderRadius: 10, background: `rgba(255,255,255,0.05)`, border: `1px solid ${C.border}`,
            display: "flex", alignItems: "center", gap: 5, fontSize: 13, touchAction: "none" }}>
            {"🔔"}
          ...
          <div onClick={() => player.notify("Settings coming soon", C.text2)} style={{ cursor: "pointer",
            padding: "6px 10px", borderRadius: 10, background: `rgba(255,255,255,0.05)`, border: `1px solid ${C.border}`,
            fontSize: 13, touchAction: "none" }}>
            {"⚙️"}
          </div>
          <div style={{ flex: 1 }} />
          {/* Multiplier badge */}
        ...
        <div style={{ display: "flex", gap: 6, marginTop: 16, borderTop: `1px solid ${C.border}`, paddingTop: 14 }}>
          {[
            { l: "Coins", v: player.coins.toLocaleString(), icon: "🪙", c: C.gold },
            { l: "Friends", v: "17,524", icon: "👥", c: C.cyan },
            { l: "Streak", v: player.dailyStreak.toString(), icon: "🔥", c: C.orange },
          ].map((s, i) => (
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 16, position: "relative", zIndex: 1 }}>
        <div style={ltPill("overview", "Overview", C.pink)} onClick={() => game.setLoyaltyTab("overview")}>Overview</div>
        <div style={ltPill("arena", "Arena", C.cyan)} onClick={() => game.setLoyaltyTab("arena")}>Arena</div>
        <div style={ltPill("social", "Social", C.purple)} onClick={() => game.setLoyaltyTab("social")}>Social</div>
      </div>

      {/* OVERVIEW TAB */}
      {game.loyaltyTab === "overview" && <>
        {/* Usage Stats Row */}
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 8, marginBottom: 14 }}>
          {[
            { l: "Puffs", v: player.playerProfile.totalPuffs, icon: "💨", c: C.cyan },
            { l: "THC", v: "143mg", icon: "🌿", c: C.green },
            { l: "CBD", v: "143mg", icon: "💊", c: C.purple },
            { l: "Beast", v: player.playerProfile.blinkerCount, icon: "🔥", c: C.orange },
          ].map((s, i) => (
        ...
        <div style={{ ...GLASS_CARD, borderRadius: 16, padding: 16, marginBottom: 12 }}>
          ...
          <div style={{ display: "flex", gap: 0, justifyContent: "center", alignItems: "center", position: "relative" }}>
            ...
            {LOYALTY_TIERS.map((t: any, i: number) => (
          ...
          {nextTier && <div style={{ marginTop: 10, padding: "8px 12px", borderRadius: 10,
            ...
            {"Next: "}<strong style={{ color: nextTier.color }}>{nextTier.name}</strong>{" -- " + ((nextTier.xpReq - player.xp) > 0 ? (nextTier.xpReq - player.xp).toLocaleString() : 0) + " XP to go"}
          </div>}
        </div>
        ...
        <div style={{ ...GLASS_CARD, borderRadius: 16, padding: 16, marginBottom: 12, position: "relative", overflow: "hidden" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
            ...
            <div style={{ fontSize: 11, color: C.orange, fontWeight: 800 }}>{"🔥 " + player.dailyStreak + " streak"}</div>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(7,1fr)", gap: 5, marginBottom: 12 }}>
            {DAILY_REWARDS.map((d: any, i: number) => {
              const isCurrent = (player.dailyStreak % 7) === i && !player.dailyCheckedIn
              const isPast = i < (player.dailyStreak % 7) || (player.dailyCheckedIn && i <= (player.dailyStreak - 1) % 7)
          ...
          <button onClick={() => player.claimDaily({ bleConnected: ble.bleConnected })} style={{ width: "100%", padding: "10px 0", borderRadius: 12, border: "none", fontWeight: 800, fontSize: 14,
            cursor: player.dailyCheckedIn ? "default" : "pointer", letterSpacing: 0.3,
            background: player.dailyCheckedIn ? C.bg3 : `linear-gradient(90deg,${C.green},${C.cyan})`,
            color: player.dailyCheckedIn ? C.text3 : "#fff",
            boxShadow: player.dailyCheckedIn ? "none" : `0 0 20px ${C.green}30, 0 4px 12px rgba(0,0,0,0.3)`,
            animation: player.dailyCheckedIn ? "none" : "claimPulse 2s ease-in-out infinite" }}>
            {player.dailyCheckedIn ? "Claimed Today" : "Claim Daily Reward"}
          </button>
        </div>
        ...
        <div style={{ ...GLASS_CARD, borderRadius: 16, padding: 16, marginBottom: 12 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
            ...
            <div style={{ fontSize: 10, fontWeight: 700, color: player.completedChallenges.length >= DAILY_CHALLENGES.length ? C.gold : C.text3 }}>
              {player.completedChallenges.length + "/" + DAILY_CHALLENGES.length}
            </div>
          </div>
          <div style={{ height: 3, borderRadius: 2, background: C.bg, overflow: "hidden", marginBottom: 10 }}>
            <div style={{ height: "100%", borderRadius: 2,
              background: player.completedChallenges.length >= DAILY_CHALLENGES.length ? `linear-gradient(90deg,${C.gold},${C.orange})` : `linear-gradient(90deg,${C.green},${C.cyan})`,
              width: `${(player.completedChallenges.length / DAILY_CHALLENGES.length) * 100}%`,
              transition: "width 0.5s ease" }} />
          </div>
          {DAILY_CHALLENGES.slice(0, 3).map((ch: any, i: number) => {
            ...
            return (
              <div key={i} style={{ display: "flex", alignItems: "center", gap: 10,
                ...
                <div style={{ fontSize: 10, fontWeight: 700, color: done ? C.text3 : C.green }}>
                  {done ? "Done" : "+" + Math.round(ch.reward * player.getCoinMultiplier(ble.bleConnected)) + " 🪙"}
                </div>
              </div>
            )
          })}
        ...
        <div style={{ ...GLASS_CARD, borderRadius: 16, padding: 16, marginBottom: 12 }}>
          ...
          {player.playerProfile.gamesPlayed > 0 ? (
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              {[
                { game: player.playerProfile.favoriteGame === "finalkick" ? "Final Kick" : "Games", result: "Won", xpLabel: "+40 XP", icon: "🎮", color: C.green, time: "2h ago" },
                { game: "Arena Session", result: player.playerProfile.gamesWon > 0 ? "Won" : "Played", xpLabel: "+20 XP", icon: "🏟️", color: C.cyan, time: "4h ago" },
                { game: "Daily Check-in", result: player.dailyCheckedIn ? "Claimed" : "Pending", xpLabel: "+20 XP", icon: "📅", color: player.dailyCheckedIn ? C.green : C.orange, time: "Today" },
      ...
      {/* ARENA TAB */}
      {game.loyaltyTab === "arena" && <>
        {/* Game Stats */}
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 8, marginBottom: 14 }}>
          {[
            { l: "Played", v: player.playerProfile.gamesPlayed, icon: "🎮", c: C.cyan },
            { l: "Win%", v: winRate + "%", icon: "🏆", c: C.green },
            { l: "Streak", v: player.bestWinStreak, icon: "🔥", c: C.orange },
            { l: "Earned", v: player.coins.toLocaleString(), icon: "🪙", c: C.gold },
          ].map((s, i) => (
            <div key={i} style={{ padding: "12px 4px", borderRadius: 14, textAlign: "center", position: "relative", overflow: "hidden",
              background: `linear-gradient(135deg, ${s.c}0a, ${s.c}04, rgba(8,8,24,0.6))`,
              backdropFilter: "blur(24px)", WebkitBackdropFilter: "blur(24px)",
        ...
        <div style={{ ...GLASS_CARD, borderRadius: 16, padding: 14, marginBottom: 12 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 8 }}>
            {LOYALTY_BADGES.map((b: any) => {
              const earned = player.earnedBadges.includes(b.id)
              return (
        ...
        <div style={{ ...GLASS_CARD, borderRadius: 16, padding: 14, marginBottom: 12 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 5, padding: "4px 10px", borderRadius: 10,
              ...
              <span style={{ fontSize: 12, fontWeight: 900, color: C.gold }}>{player.coins.toLocaleString()}</span>
            </div>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 8 }}>
            {SHOP_ITEMS.map((item: any, i: number) => {
              const bought = player.ownedItems.includes(item.id)
              const canAfford = player.coins >= item.price
              const tierLocked = item.tier && tier.idx < LOYALTY_TIERS.findIndex((t: any) => t.name === item.tier)
              return (
                <div key={i} onClick={() => !bought && !tierLocked && player.buyShopItem(item)} style={{ padding: 10, borderRadius: 14, textAlign: "center",
                  ...
                  transition: "all 0.3s ease", touchAction: "none" }}>
                  {tierLocked && !bought && <div style={{ position: "absolute", inset: 0,
                    background: "rgba(5,5,16,0.7)", backdropFilter: "blur(2px)",
                    display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column" as const, zIndex: 2, borderRadius: 14 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 8, marginBottom: 12 }}>
          {[
            { label: "Total Puffs", value: player.playerProfile.totalPuffs, icon: "💨", color: C.cyan },
            { label: "Blinkers", value: player.playerProfile.blinkerCount, icon: "💀", color: C.red },
            { label: "Puff Time", value: Math.floor(player.playerProfile.totalPuffTime / 60) + "m", icon: "⏱️", color: C.purple },
          ].map((st, i) => (
      ...
      {game.loyaltyTab === "social" && <>
        ...
        <div style={{ ...GLASS_CARD, borderRadius: 16, padding: 16, marginBottom: 12 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
            ...
            <div style={{ fontSize: 10, color: C.green, fontWeight: 700 }}>
              {MOCK_FRIENDS.filter((f: any) => f.status === "online").length + " online"}
            </div>
          </div>
          {MOCK_FRIENDS.map((f: any, i: number) => (
            <div key={i} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 10px", borderRadius: 12, marginBottom: 6,
              ...
              {f.status === "online" && (
                <div onClick={() => player.notify("Challenge coming soon", C.cyan)} style={{ cursor: "pointer",
                  padding: "5px 10px", borderRadius: 8, fontSize: 9, fontWeight: 700,
                  background: `${C.cyan}12`, border: `1px solid ${C.cyan}20`, color: C.cyan, touchAction: "none" }}>
                  Challenge
                </div>
              )}
            </div>
        ...
        <div style={{ ...GLASS_CARD, borderRadius: 16, padding: 16, marginBottom: 12 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
            {MOCK_COMMUNITIES.map((c: any, i: number) => (
              <div key={i} onClick={() => player.notify("Community hub coming soon", C.purple)} style={{ cursor: "pointer",
        ...
        <div style={{ ...GLASS_CARD, borderRadius: 16, padding: 16, marginBottom: 12 }}>
          ...
          {MOCK_COLLECTIONS.map((c: any, i: number) => (
