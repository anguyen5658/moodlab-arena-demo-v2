...
const PP_RALLY = ['Rally {n}! Lungs are an engine 🫁🏎️', 'Rally {n}! INSANE 🤯', 'Rally {n}! Crowd ROARING 📣', '{n} hits! Legendary 🔥🔥🔥', 'Rally {n}! UNBREAKABLE 💪'];
...
const PP_RALLY_ROASTS = ['That rally was FILTHY 🤮💨', 'Someone clip that 📱', 'W gameplay fr fr 🔥', 'Chat is going WILD 💀', 'Bro is on another level 😤', 'These hands are INSANE 👏', 'L for the AI 😂', 'Clean. Very clean. 🏓', 'DISGUSTING return 🫠', 'That angle was ILLEGAL 🚔', 'AI is SHOOK right now 😱'];
...
export default function PuffPong() {
  ...
  const intervalRef = useRef(null); // paddle drift interval
  const powerIntervalRef = useRef(null);
  const powerStartRef = useRef(0);

  // All mutable game state in a single ref object (like the monolith's ppG)
  const G = useRef({
    bx: 50, by: 50, dx: 2.2, dy: 1.2,
    py: 50, ay: 50,
    rally: 0, trail: [],
    scoreY: 0, scoreA: 0,
    paused: false, power: 0, charging: false,
  ...
  const spawnParticles = useCallback((x, y, color, count) => {
    ...
    for (let i = 0; i < count; i++) {
      g.particles.push({ x, y, vx: (Math.random() - 0.5) * 6, vy: (Math.random() - 0.5) * 6, life: 1, decay: 0.02 + Math.random() * 0.03, r: 2 + Math.random() * 3, color });
    }
  }, []);
  ...
  const drawCanvas = useCallback(() => {
    ...
    // Center line
    ctx.setLineDash([6, 6]); ctx.strokeStyle = 'rgba(255,255,255,0.12)'; ctx.lineWidth = 2;
    ...
    if (spd > 3.5) {
      ...
      ctx.strokeStyle = `rgba(255,255,255,${Math.min(0.3, (spd - 3.5) * 0.1)})`; ctx.lineWidth = 1;
      for (let i = 0; i < 3; i++) {
    ...
    ctx.beginPath(); ctx.roundRect(8, pyPx - padH / 2, PP_PADDLE_W, padH, 5); ctx.fill();
    ctx.shadowBlur = 0;
    ...
    ctx.beginPath(); ctx.roundRect(W - 8 - PP_PADDLE_W, ayPx - aiPadH / 2, PP_PADDLE_W, aiPadH, 5); ctx.fill();
    ctx.shadowBlur = 0;

    // Ball
    const ballX = g.bx / 100 * W, ballY = g.by / 100 * H;
    const isSmash = g.smash && g.power >= 80;
    ctx.shadowColor = isSmash ? '#FFD93D' : '#00E5FF'; ctx.shadowBlur = isSmash ? 25 : 15;
    const ballGrad = ctx.createRadialGradient(ballX, ballY, 0, ballX, ballY, PP_BALL_R + 2);
    if (isSmash) { ballGrad.addColorStop(0, '#FFFFFF'); ballGrad.addColorStop(0.5, '#FFD93D'); ballGrad.addColorStop(1, 'rgba(255,90,90,0.6)'); }
    else { ballGrad.addColorStop(0, '#FFFFFF'); ballGrad.addColorStop(0.6, 'rgba(0,229,255,0.9)'); ballGrad.addColorStop(1, 'rgba(0,229,255,0.3)'); }
    ...
    if (g.impact && Date.now() - g.impact.t < 250) {
      ...
      ctx.fillStyle = impGrad; ctx.beginPath(); ctx.arc(ix, iy, 25, 0, Math.PI * 2); ctx.fill();
    }
  ...
  const startLoop = useCallback(() => {
    ...
    const loop = (now) => {
      if (g.paused) return;
      const dt = Math.min((now - g.lastT) / 16.667, 3); g.lastT = now;
      let nx = g.bx + g.dx * dt, ny = g.by + g.dy * dt, ndx = g.dx, ndy = g.dy;
      g.trail.push({ x: g.bx, y: g.by, age: 0 }); if (g.trail.length > 20) g.trail.shift(); g.trail.forEach(t => t.age++);
      ...
      if (nx <= 6 && g.bx > 6) {
        ...
        if (hd < padH100 / 2 + 2) {
          ...
          ndy = g.dy + hitOffset * 2.5; nx = 7;
          g.rally++; playFx('pong_hit');
          g.impact = { x: 6, y: ny, t: Date.now() };
          spawnParticles(6, ny, isCenter || powerBoost >= 2 ? '#FFD93D' : '#00E5FF', powerBoost >= 2 ? 20 : 10);
          if (powerBoost >= 2) {
            g.smashText = Date.now(); setSmash(true); triggerShake(); setComment(pick(PP_SMASH));
            addChat(pick(['BlinkerBaron', 'NeonQueen', 'CloudNine']), pick(['SMASH CITY 💣', 'THAT WAS DISGUSTING 💀', 'BRO 2X SPEED?! 😱']), '#FFD93D');
            setTimeout(() => setSmash(false), 500);
          } else if (isCenter) { setComment(pick(PP_SMASH)); triggerShake(); }
          else { setComment(pick(PP_HIT)); }
          if (g.rally > 0 && g.rally % 5 === 0) {
            addChat('Announcer', pick(PP_RALLY).replace('{n}', String(g.rally)), '#FFD93D');
            if (g.rally % 10 === 0) playFx('crowd');
            if (now - lastChat > 3000) { addChat(pick(['CloudChaser', 'BlazedPanda', 'SeshGremlin']), pick(PP_RALLY_ROASTS), '#C084FC'); lastChat = now; }
          }
          setRally(g.rally); setSpeed(spd);
          g.power = 0; g.charging = false; setPower(0); setCharging(false);
        } else {
          g.scoreA++; setScore({ you: g.scoreY, ai: g.scoreA }); triggerShake(); triggerFlash('miss'); playFx('pong_score');
          setComment(pick(PP_SA)); addChat('PuffBot', pick(PP_SA), '#FF5A5A');
          g.rally = 0; setRally(0); g.power = 0; setPower(0);
          if (g.scoreA >= PP_WIN_SCORE) {
            g.paused = true; setPhase('result');
            playFx('lose'); addChat('PuffBot', 'AI wins! Better luck next puff! 🤖', '#FF5A5A');
            awardGame('puffpong', 'lose'); return;
          }
      ...
      if (nx >= 94 && g.bx < 94) {
        ...
        if (hd < 10) {
          ndx = -Math.abs(ndx) * 1.03 * rallyBoost; ndy = g.dy + (ny - g.ay) * 0.12; nx = 93;
          g.rally++; setRally(g.rally); playFx('pong_hit'); g.impact = { x: 94, y: ny, t: Date.now() };
          spawnParticles(94, ny, '#FF5A5A', 8);
        } else {
          g.scoreY++; setScore({ you: g.scoreY, ai: g.scoreA }); triggerFlash('goal'); triggerShake(); playFx('pong_score');
          setComment(pick(PP_SY)); addChat('PuffBot', pick(PP_SY), '#00E5FF');
          g.rally = 0; setRally(0); g.power = 0; setPower(0);
          if (g.scoreY >= PP_WIN_SCORE) {
            g.paused = true; setPhase('result');
            playFx('win'); spawnConfetti(40); addChat('PuffBot', 'CHAMPION! Puff-powered victory! 🏆💨', '#00E5FF');
            awardGame('puffpong', 'win'); return;
          }
          nx = 50; ny = 50; ndx = -2.2; ndy = 1.2 * (Math.random() > 0.5 ? 1 : -1); g.trail = [];
        }
      }
    ...
    rafRef.current = requestAnimationFrame(loop);
  }, [playFx, triggerShake, triggerFlash, spawnParticles, addChat, awardGame, drawCanvas, spawnConfetti]);
  ...
  const startGame = useCallback(() => {
    ...
    setScore({ you: 0, ai: 0 }); setRally(0); setSpeed(0);
    setPower(0); setCharging(false); setSmash(false); setComment('');
    setIntro(1); setPhase('intro'); playFx('whistle');
    addChat('PuffBot', 'Neon Puff Pong starting! First to 5 wins! 🏓', '#00E5FF');
    setTimeout(() => setIntro(2), 500);
    setTimeout(() => setIntro(3), 1000);
    setTimeout(() => setIntro(4), 1500);
    setTimeout(() => {
      setIntro(5); setComment('SERVE!'); playFx('crowd');
      addChat('CloudChaser', "LET'S GOOO! 🔥🔥", '#FF6B8A');
      setTimeout(() => { setPhase('playing'); phaseRef.current = 'playing'; setIntro(0); setComment(''); startLoop(); }, 600);
    }, 2000);
  }, [cleanup, playFx, addChat, startLoop]);
  ...
  const puffDown = useCallback(() => {
    setPuffHeld(true); setCharging(true);
    const g = G.current; g.charging = true;
    powerStartRef.current = Date.now();
    if (powerIntervalRef.current) clearInterval(powerIntervalRef.current);
    powerIntervalRef.current = setInterval(() => {
      const elapsed = (Date.now() - powerStartRef.current) / 1000;
      const pw = getPuffPower(elapsed);
      g.power = pw; setPower(pw);
    }, 50);
    if (intervalRef.current) clearInterval(intervalRef.current);
  ...
  const puffUp = useCallback(() => {
    setPuffHeld(false); setCharging(false);
    const g = G.current; g.charging = false;
    if (powerIntervalRef.current) { clearInterval(powerIntervalRef.current); powerIntervalRef.current = null; }
    if (intervalRef.current) clearInterval(intervalRef.current);
    intervalRef.current = setInterval(() => { if (!g.paused) { g.py = Math.min(92, g.py + 0.5); } }, 80);
  }, []);
  ...
  // BLE: slot 0
  useEffect(() => {
    setBLEHandlers(0, () => puffDown(), () => puffUp());
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 100, display: 'flex', flexDirection: 'column', touchAction: 'none' }}>
      ...
      <div
        ...
        onClick={(e) => {
          if (phaseRef.current !== 'playing' || e.target.closest('[data-btn]')) return;
      ...
      >
        ...
        {isIntro && (
          <div style={{ position: 'absolute', inset: 0, zIndex: 20, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: 'rgba(6,16,30,0.85)', backdropFilter: 'blur(6px)' }}>
            ...
            {intro >= 2 && <div style={{ fontSize: 11, color: C.text2, fontWeight: 600, animation: 'fadeIn 0.5s ease', marginBottom: 12 }}>Tap to move paddle | Hold puff for power shot</div>}
            {intro >= 3 && (
              <div style={{ display: 'flex', gap: 16, animation: 'fadeIn 0.5s ease' }}>
                <div style={{ textAlign: 'center' }}><div style={{ width: 12, height: 50, borderRadius: 6, background: C.cyan, boxShadow: `0 0 15px ${C.cyan}80` }} /><div style={{ fontSize: 8, color: C.cyan, marginTop: 4, fontWeight: 700 }}>YOU</div></div>
                <div style={{ fontSize: 18, fontWeight: 900, color: C.text3, alignSelf: 'center' }}>VS</div>
                <div style={{ textAlign: 'center' }}><div style={{ width: 12, height: 40, borderRadius: 6, background: '#FF5A5A', boxShadow: '0 0 15px rgba(255,90,90,0.5)' }} /><div style={{ fontSize: 8, color: '#FF5A5A', marginTop: 4, fontWeight: 700 }}>AI</div></div>
              </div>
            )}
            {intro >= 4 && <div style={{ marginTop: 16, animation: 'fadeIn 0.5s ease' }}><div style={{ fontSize: 10, color: C.text3, fontWeight: 600 }}>First to {PP_WIN_SCORE} wins</div></div>}
            {intro === 5 && <div style={{ marginTop: 16, fontSize: 32, fontWeight: 900, color: C.gold, letterSpacing: 6, textShadow: `0 0 20px ${C.gold}` }}>SERVE!</div>}
          </div>
        )}
        ...
        {/* Controls */}
        {phase === 'playing' && (
          <div style={{ position: 'absolute', bottom: '8%', left: 0, right: 0, zIndex: 30, padding: '5px 10px 9px' }}>
            <div style={{ display: 'flex', gap: 6, alignItems: 'center', justifyContent: 'center' }}>
              <div data-btn="true"
                onMouseDown={() => movePaddle(-1)}
                onTouchStart={(e) => { e.preventDefault(); movePaddle(-1); }}
                style={{ flex: 1, maxWidth: 80, padding: '10px 0', borderRadius: 10, cursor: 'pointer', textAlign: 'center', background: 'rgba(0,229,255,0.08)', border: '1px solid rgba(0,229,255,0.2)', fontSize: 13, fontWeight: 800, color: C.cyan, userSelect: 'none', WebkitUserSelect: 'none' }}>↑ UP</div>
              <div data-btn="true"
                onMouseDown={puffDown} onMouseUp={puffUp} onMouseLeave={puffUp}
                onTouchStart={(e) => { e.preventDefault(); puffDown(); }}
                onTouchEnd={puffUp}
                style={{ flex: 1, maxWidth: 120, padding: '12px 0', borderRadius: 12, cursor: 'pointer', textAlign: 'center', background: puffHeld ? 'rgba(0,229,255,0.2)' : 'rgba(0,229,255,0.06)', border: `2px solid ${puffHeld ? C.cyan : C.cyan + '40'}`, fontSize: 12, fontWeight: 900, color: C.cyan, transition: 'all 0.15s', transform: puffHeld ? 'scale(0.95)' : 'scale(1)', userSelect: 'none', WebkitUserSelect: 'none' }}>
                {puffHeld ? '💨 CHARGING...' : '🏓 HOLD PUFF'}
              </div>
              <div data-btn="true"
                onMouseDown={() => movePaddle(1)}
                onTouchStart={(e) => { e.preventDefault(); movePaddle(1); }}
                style={{ flex: 1, maxWidth: 80, padding: '10px 0', borderRadius: 10, cursor: 'pointer', textAlign: 'center', background: 'rgba(0,229,255,0.08)', border: '1px solid rgba(0,229,255,0.2)', fontSize: 13, fontWeight: 800, color: C.cyan, userSelect: 'none', WebkitUserSelect: 'none' }}>↓ DOWN</div>
            </div>
            <div style={{ fontSize: 7, color: C.text3, marginTop: 3, textAlign: 'center', opacity: 0.5 }}>TAP top/bottom = move | HOLD PUFF = charge power shot</div>
          </div>
        )}

        {/* Result overlay */}
        {phase === 'result' && (
          <div style={{ position: 'absolute', inset: 0, zIndex: 30, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: 'rgba(5,5,16,0.85)', backdropFilter: 'blur(10px)', animation: 'fadeIn 0.5s ease' }}>
            <div style={{ fontSize: 24, fontWeight: 900, color: score.you > score.ai ? C.cyan : '#FF5A5A', letterSpacing: 3, marginBottom: 4 }}>{score.you > score.ai ? 'YOU WIN! 🏆' : 'AI WINS 🤖'}</div>
            ...
            <div style={{ fontSize: 11, color: C.text2, marginBottom: 12 }}>{score.you > score.ai ? 'Puff Pong champion! Powered by THC 💨🏆' : 'Better luck next puff! 💨'}</div>
            <div style={{ display: 'flex', gap: 10 }}>
              <div data-btn="true" onClick={() => { setPhase(null); startGame(); }} style={{ padding: '12px 28px', borderRadius: 14, cursor: 'pointer', background: 'rgba(0,229,255,0.12)', border: '2px solid rgba(0,229,255,0.3)', fontSize: 14, fontWeight: 800, color: C.cyan }}>🔄 Rematch</div>
              <div data-btn="true" onClick={() => { cleanup(); navigate('/arcade'); }} style={{ padding: '12px 28px', borderRadius: 14, cursor: 'pointer', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)', fontSize: 14, fontWeight: 800, color: C.text3 }}>Done</div>
            </div>
