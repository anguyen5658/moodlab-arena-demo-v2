...
const PLAYER_COUNTS = {
  finalkick:2100, finalkick2:356, wildwest:720, russian:167, balloon:145,
  puffpong:289, rhythm:134, tugofwar:312, hotpotato:890, hooked:410,
  rps:540, beatdrop:245, puffclock:310, pufflimbo:178, puffderby:420,
  tankwar:680, fishwar:520, rooftoppuff:430,
};

const TOURNAMENTS = [
  { name:'FK1 World Cup 2026', emoji:'🏆', game:'Final Kick', prize:'50,000 coins', color:C.gold,   status:'LIVE',     players:312, round:'Round of 16' },
  { name:'The Outlaw Circuit',  emoji:'🤠', game:'Wild West',  prize:'25,000 coins', color:C.orange, status:'OPEN',     players:148, round:'Qualifying' },
...
const TRIED = ['finalkick','wildwest','puffpong','tugofwar','rps'];
...
export default function ArcadeZone() {
  ...
  const featuredGame = hotGames[Math.floor(Date.now() / 4000) % Math.max(hotGames.length, 1)] || PLAY_GAMES[0];
  ...
  const heroSlides = [
    { emoji: featuredGame.emoji, title: featuredGame.name, sub: `${featuredGame.type} · ${PLAYER_COUNTS[featuredGame.id] || 0} playing`, color: featuredGame.color, badge: featuredGame.hot ? '🔥 HOT' : '', action: () => navigate(`/arcade/${featuredGame.id}`) },
    { emoji: '🏆', title: activeTourney.name, sub: `${activeTourney.players} entered · ${activeTourney.round}`, color: C.gold, badge: activeTourney.status, action: null },
    ...
    { emoji: '⚡', title: 'Quick Play', sub: 'Jump into a random game instantly!', color: C.lime, badge: 'INSTANT', action: () => navigate(`/arcade/${PLAY_GAMES[Math.floor(Math.random() * PLAY_GAMES.length)].id}`) },
  ];
  ...
  return (
    <div style={{ height: '100%', overflowY: 'auto', background: C.bg, paddingBottom: 80 }}>
      ...
      <div style={{ position: 'relative', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 320, pointerEvents: 'none', background: `radial-gradient(ellipse at 50% 0%, ${C.cyan}15, transparent 60%)` }}>
          <div style={{ position: 'absolute', inset: 0, backgroundImage: `linear-gradient(${C.cyan}06 1px, transparent 1px), linear-gradient(90deg, ${C.cyan}06 1px, transparent 1px)`, backgroundSize: '40px 40px', opacity: 0.5 }} />
        </div>

        {/* Hero slider */}
        <div onClick={slide.action} style={{ padding: '12px 14px 10px', cursor: slide.action ? 'pointer' : 'default', position: 'relative' }}>
        ...
        <div style={{ padding: '0 14px 8px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '5px 12px', borderRadius: 100, background: `${C.cyan}06`, border: `1px solid ${C.cyan}10` }}>
            ...
            <span style={{ fontSize: 9, color: C.text2, fontWeight: 600, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
              {['🔥 ' + featuredGame.name + ' trending · ' + (PLAYER_COUNTS[featuredGame.id] || 0) + ' playing', '🏆 ' + activeTourney.name + ' ' + activeTourney.round + ' starting', '⚡ ' + ACTIVITY[0].text, '🎯 ' + ACTIVITY[1].text, '💣 ' + ACTIVITY[4].text][Math.floor(tick / 3) % 5]}
            </span>
          </div>
        </div>
        ...
        <div style={{ padding: '0 14px 8px' }}>
          <div style={{ display: 'flex', gap: 6 }}>
            {[
              { v: MY_STATS.gamesPlayed, l: 'Played',  color: C.cyan },
              { v: MY_STATS.winRate + '%', l: 'Win Rate', color: C.green },
              { v: MY_STATS.coinsWon.toLocaleString(), l: 'Coins', color: C.gold },
              { v: '🔥' + MY_STATS.streak, l: 'Streak', color: C.orange },
            ].map(s => (
        ...
        <div style={{ padding: '0 14px' }}>
          <div style={{ display: 'flex', borderBottom: `1px solid rgba(255,255,255,0.06)`, marginBottom: 10 }}>
          ...
          {tab === 'games' && (
            <div style={{ marginBottom: 4 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                {smartSorted.map((g, i) => {
                  const isNew = !TRIED.includes(g.id);
                  const count = PLAYER_COUNTS[g.id] || 30;
                  return (
                    <div
                      key={g.id}
                      onClick={() => { playFx('select'); navigate(`/arcade/${g.id}`); }}
                      style={{ padding: '12px 10px', borderRadius: 14, cursor: 'pointer', position: 'relative', overflow: 'hidden', background: `radial-gradient(ellipse at 50% 0%, ${g.color}10, rgba(10,16,30,1) 70%)`, border: `1px solid ${g.color}15`, transition: 'all 0.3s', animation: `fadeIn 0.3s ease ${i * 0.04}s both` }}
                    >
          ...
          {tab === 'tournaments' && (
            <div style={{ marginBottom: 14 }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6 }}>
                {TOURNAMENTS.map((t, i) => (
                  <div key={i} onClick={() => { playFx('select'); }} style={{ padding: '10px 10px', borderRadius: 12, cursor: 'pointer', background: `radial-gradient(ellipse at 30% 20%, ${t.color}10, rgba(10,16,30,1) 70%)`, border: `1px solid ${t.color}20` }}>
          ...
          {tab === 'activity' && (
            <div style={{ marginBottom: 14, borderRadius: 14, overflow: 'hidden', border: `1px solid ${C.cyan}10`, background: `${C.cyan}03` }}>
              {ACTIVITY.map((a, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', borderBottom: i < ACTIVITY.length - 1 ? '1px solid rgba(255,255,255,0.04)' : 'none' }}>
          ...
          {tab === 'stats' && (
            <div style={{ marginBottom: 14 }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 10 }}>
                {[
                  { v: MY_STATS.gamesPlayed, l: 'Games Played', color: C.cyan },
                  { v: MY_STATS.winRate + '%', l: 'Win Rate', color: C.green },
                  { v: MY_STATS.coinsWon.toLocaleString(), l: 'Coins Won', color: C.gold },
                  { v: '🔥 ' + MY_STATS.streak, l: 'Win Streak', color: C.orange },
                ].map(s => (
                  <div key={s.l} style={{ padding: '12px', borderRadius: 12, background: `${s.color}06`, border: `1px solid ${s.color}12`, textAlign: 'center' }}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <div style={{ flex: 1, padding: '10px 12px', borderRadius: 12, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', gap: 8 }}>
