...
import { SHOW_GAMES } from '../../constants/games.js';
...
export default function StageZone() {
  ...
  const infoItem = INFO_ITEMS[Math.floor(tick / 3) % INFO_ITEMS.length];
  ...
  return (
    <div style={{ height: '100%', overflowY: 'auto', background: C.bg, paddingBottom: 80 }}>
      ...
      <div style={{ position: 'relative', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', top: 0, left: '50%', transform: 'translateX(-50%)', width: 500, height: 280, pointerEvents: 'none', background: `radial-gradient(ellipse at 50% 0%, ${C.gold}14, transparent 50%)` }} />

        <div style={{ padding: '12px 14px', position: 'relative', zIndex: 1 }}>
          {/* Hero slider */}
          <div onClick={() => navigate(`/stage/${liveShow.id}`)} style={{ marginBottom: 10, cursor: 'pointer' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, position: 'relative', zIndex: 1 }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    ...
                    {slide.badge && <span style={{ fontSize: 6, fontWeight: 900, padding: '2px 6px', borderRadius: 4, background: slide.badge === 'LIVE' ? `${C.red}18` : `${slide.color}12`, color: slide.badge === 'LIVE' ? C.red : slide.color, border: `1px solid ${slide.badge === 'LIVE' ? C.red : slide.color}25` }}>{slide.badge}</span>}
                  </div>
                  <div style={{ fontSize: 10, color: C.text3, marginTop: 2 }}>{slide.sub}</div>
                </div>
              </div>
              <div style={{ display: 'flex', gap: 4, justifyContent: 'center', marginTop: 8 }}>
                {stageSlides.map((_, i) => <div key={i} style={{ width: i === slideIdx ? 16 : 5, height: 5, borderRadius: 3, background: i === slideIdx ? slide.color : `${C.text3}30`, transition: 'all 0.3s' }} />)}
              </div>
            </div>
          </div>

          {/* Feed bar */}
          <div style={{ padding: '6px 12px', borderRadius: 100, marginBottom: 10, background: `${infoItem.color}06`, border: `1px solid ${infoItem.color}10`, display: 'flex', alignItems: 'center', gap: 6 }}>
            <div style={{ width: 4, height: 4, borderRadius: '50%', background: C.green, animation: 'pulse 1.5s infinite' }} />
            <span style={{ fontSize: 9, fontWeight: 600, color: infoItem.color }}>{infoItem.text}</span>
          </div>

          {/* Stat strip */}
          <div style={{ display: 'flex', gap: 6, marginBottom: 12 }}>
            {[{ v: 23, l: 'Shows', color: C.gold }, { v: 12, l: 'Wins', color: C.green }, { v: '3,200', l: 'Coins', color: C.cyan }, { v: '4x', l: 'Roles', color: C.red }].map(s => (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6 }}>
            {SHOW_GAMES.map((g, i) => {
              ...
              return (
                <div key={g.id} onClick={() => { playFx('select'); navigate(`/stage/${g.id}`); }} style={{ padding: '10px 8px', borderRadius: 12, cursor: 'pointer', textAlign: 'center', position: 'relative', background: isLive ? 'rgba(255,50,50,0.06)' : 'rgba(255,255,255,0.02)', border: `1px solid ${isLive ? 'rgba(255,50,50,0.2)' : 'rgba(255,255,255,0.06)'}` }}>
                  {isLive && (
                  ...
                  {g.id === 'vibecheck' && <div style={{ position: 'absolute', top: 4, left: 4, fontSize: 6, fontWeight: 900, color: '#fff', padding: '1px 5px', borderRadius: 3, background: C.gold }}>WC Edition</div>}
